AI usage: Claude Sonnet 4.5/ Grok 

Before using the AI model for code generation/code modification, the following prompt is fed into the AI model for role clarification:

```text
# Identity

You are an expert database systems developer and PostgreSQL specialist working on a university coursework project for a gig management system. You excel at designing relational database schemas, implementing complex SQL queries with CTEs and window functions, enforcing business rules through triggers and constraints, and integrating database operations with Java JDBC applications.

# Instructions

* Focus on PostgreSQL-specific features and best practices
* Implement business rules through database constraints, triggers, and stored procedures rather than application logic where possible
* Use appropriate data types, constraints, and indexing for performance and data integrity
* Design queries that are efficient, readable, and follow SQL standards
* Ensure proper transaction management and error handling in both SQL and Java code
* Follow the coursework specification requirements precisely while demonstrating advanced database concepts
* Provide clear explanations of design decisions and trade-offs
* Always make sure the schema and result compiled never violate the instructions listed in `business.md`
* Make sure for every implementation of code segment, implement a fall back policy to ensure the database integrity. 

# Expertise Areas

* Relational database design and normalization
* Complex SQL queries with CTEs, joins, aggregations, and window functions
* PostgreSQL triggers, functions, and stored procedures
* Java JDBC integration with proper connection management
* Database constraint design and enforcement
* Performance optimization through indexing and query design
...
<task_description>
Implement Task 2: Create a new gig with multiple acts, ensuring all business rules are enforced
</task_description>

<expected_approach>
1. Use transaction with proper rollback on violations
2. Validate inputs and existing data constraints
3. Insert gig record first, then act performances
4. Use database triggers to enforce interval rules and act availability
5. Handle exceptions and provide meaningful error messages
</expected_approach>

<task_description>
Design Task 5: Calculate tickets needed to sell for each gig to break even
</task_description>

<expected_approach>
1. Use CTEs to calculate total costs (act fees + venue hire)
2. Calculate total revenue from existing ticket sales
3. Find minimum ticket price for each gig
4. Use CEIL function for proper rounding of ticket calculations
5. Order results appropriately and handle edge cases
</expected_approach>
```
- It is understandable that the code generated by the AI agent requires user engineers' careful examination. Thus the author of this database has adopted the approach with the following work flow: 
1. Implement database schema with the minimum work, make the table "exists". 
2. Carefully construct the logic for each task before asking the AI agent to generate code that function like the given instructions. 
3. Require the AI agent to provide a comprehensive code review before implement it, point out if there is any places that can be microoptimized
4. Compile it and run test everytime after modifications: 
   - If the code works, push it to github
   - If not, insert some print error messages for easier debugging, and analyze the places that can lead to such error
5. Draft extreme test cases with multiple violations/ edge cases to test the ability of the database system, make sure the test cases strictly follow the document `business.md`

# Table Schema Definition: 
- All tables have appropriate primary keys
   - e.g. ACT_GIG uses compostie primary key (actid, gigid, ontime) to allow multiple performance by same act. 
- Foreign keys are proplery defined using "ON DELETE CASCADE"
- Checked constraints: 
   - Non-negative results 
   - Business rules constraints
- Setting up sequence for auto-increment for the database needed (act_actid_seq etc.)
...
### Key Constraints and Triggers aligning with business rules: 
To ensure each business rule is enforced, a helper function has been implemented for every rule. These helper functions are defined at the top of "GigSystem.java" and are invoked as needed within the relevant task implementations.
...
- The gig datetime represents when the event begins, so the first performance must align with this
...
- Ensures smooth scheduling without conflicts while allowing back-to-back performances
...
- Allows acts to perform multiple times at a gig (e.g., first half, interval, second half) but requires breaks between performances
...
- Provides sufficient time for acts to travel between venues
...
- Provides time for venue staff to clean and prepare between events
...
- Ensures gigs have minimum duration for value and logistics
...
- Acts receive one fee per gig regardless of number of performances
...
Indexes are created on frequently queried columns for micro-optimization:
...
### Potential Table Structure Improvements

#### 1. **Normalization**
**Current:** `ACT.genre` stores genre as a VARCHAR(100) string
- Create a separate GENRE table with foreign key reference

```sql
CREATE TABLE GENRE (
    genreid INTEGER PRIMARY KEY,
    genrename VARCHAR(50) UNIQUE NOT NULL,
    noise_level CHAR(1) CHECK (noise_level IN ('L', 'M', 'H')) -- For Business Rule 14 automation
);

ALTER TABLE ACT ADD COLUMN genreid INTEGER REFERENCES GENRE(genreid);
```
- Eliminates duplicate genre strings
- Enables easier genre-based reporting
- We cane easily automates noise level determination for Business Rule 14
- Supports genre hierarchy or categorization (Rock -> blue rock/ country rock etc.)

#### 2. **Venue Capacity History**
**Current:** VENUE.capacity is static
- Add temporal capacity tracking
...
CREATE TABLE VENUE_CAPACITY_HISTORY (
    ...
    reason VARCHAR(255), -- e.g., "Temporary reduction due to renovations"
    PRIMARY KEY (venueid, effective_date)
);
```
- Tracks capacity changes over time
- Supports venue renovations or temporary restrictions
- Enables historical ticket sales analysis

#### 3. **Enhanced Ticket Pricing**
**Current:** GIG_TICKET has simple price per type per gig
- Add pricing tiers and time-based pricing

```sql
ALTER TABLE GIG_TICKET ADD COLUMN price_tier VARCHAR(20) DEFAULT 'Standard';
ALTER TABLE GIG_TICKET ADD COLUMN early_bird_deadline TIMESTAMP;
ALTER TABLE GIG_TICKET ADD COLUMN early_bird_price INTEGER CHECK (early_bird_price >= 0);
```
- Supports dynamic pricing strategies for marketing/ business development
- Enables early bird discounts
- Accommodates different seating tiers
- Future-proofs for advanced pricing models

#### 4. **Composite Key Optimization**
**Current:** ACT_GIG uses (actid, gigid, ontime) composite primary key
- Consider surrogate key for better performance
- Implement system-generated unique identifiers (like auto-increment integers),  for simpler, faster joins and stable primary keys. 

```sql
ALTER TABLE ACT_GIG ADD COLUMN act_gig_id INTEGER PRIMARY KEY DEFAULT nextval('act_gig_id_seq');

ALTER TABLE ACT_GIG ADD CONSTRAINT unique_act_gig_performance UNIQUE (actid, gigid, ontime);
```
